"""
DeepWikiからMarkdownへのコンバーター用のHTMLスクレイパー。

このスクレイパーはDeepWikiサイトからHTMLコンテンツを取得し、Markdownに変換します。
"""
...
class HtmlScraper(BaseScraper):
    """
    DeepWikiサイトからHTMLコンテンツを取得し、Markdownに変換するためのスクレイパー。
    
    このスクレイパーはDeepWikiサイトからHTMLコンテンツを取得し、markdownifyを使用して
    Markdown形式に変換します。
    """
    ...
    def scrape(self) -> List[Dict[str, Any]]:
        ...
        # メインコンテンツを取得
        html_content = self._fetch_content(self.config.url)
        if not html_content:
            logger.error(f"Failed to fetch content from {self.config.url}")
            return results

        # HTMLをMarkdownに変換
        md_content = self._convert_to_markdown(html_content)

        # メインコンテンツを保存
        filename = self._get_filename_from_url(self.config.url)
        filepath = self.save_content(filename, md_content)
        results.append({
            "url": self.config.url,
            "filename": filename,
            "filepath": filepath
        ...
        # ナビゲーション項目を抽出して処理
        nav_items = self.extract_navigation(html_content)
        for item in nav_items:
            ...
            # コンテンツを取得して変換
            item_html = self._fetch_content(url)
            if not item_html:
                logger.warning(f"Failed to fetch content from {url}")
                continue

            item_md = self._convert_to_markdown(item_html)

            # コンテンツを保存
            item_filename = item.get("title", self._get_filename_from_url(url))
            item_filepath = self.save_content(item_filename, item_md)
            results.append({
                "url": url,
    ...
    def extract_navigation(self, content: str) -> List[Dict[str, Any]]:
        """
        HTMLコンテンツからナビゲーションを抽出します。
        
        Args:
            content (str): ナビゲーションを抽出するHTMLコンテンツ。
