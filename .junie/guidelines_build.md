...
- Development install / import options
  - To make sources importable, either:
    - Temporarily set PYTHONPATH to src (Unix): PYTHONPATH=src python -c "from deepwiki import cli; print(cli.__name__)"
    - On Windows (PowerShell): $env:PYTHONPATH="src"; python -c "from deepwiki import cli; print(cli.__name__)"
...
- Runtime working directory / output behavior
  - When input is a DeepWiki URL, output is split by H1 and saved under .deepwiki/<username>/<library>/<section>.md, and an index is generated at .deepwiki/<username>/<library>.md (see src/deepwiki/cli.py _write_output).
...
- Chat feature (Devin API integration)
  ...
  - Config generation (src/deepwiki/chat.py: load_or_create_config):
